...
- Application Next.js 16 (App Router), TypeScript, React 19, Tailwind CSS 4 et Node.js 24 LTS (déclaré dans `.nvmrc` et `package.json`, imposé par les scripts npm et le préflight des Runs).
- `src/app/page.tsx` et `src/app/[projectSlug]/page.tsx` restent les routes et composent l’App Shell ReUI et les vues Project (Tasks, Sequences, Instructions, Agents, Settings).
- `src/modules/` possède l’interface par domaine : `app-shell/` (navigation), `projects/` (liste, contexte, composition et Settings), `tasks/` (éditeur, liste et historique), `sequences/` (éditeur et liste), `runs/` (Sheet, queue et Run Inspector), `agents/` et `instructions/`. Les hooks de chargement et les règles de présentation des Tasks et Sequences vivent dans leur module; les appels HTTP utilisés par plusieurs modules sont dans `src/lib/client-request.ts`.
- `src/components/ui/` contient les primitives shadcn ; `src/components/reui/` contient les primitives ReUI. `src/components/theme-provider.tsx` et `src/components/dev/` restent des éléments transversaux ou de développement. `src/hooks/` et `src/lib/` sont réservés aux usages réellement transversaux; `src/types/` conserve les contrats partagés avec `backend/` et les tests. Voir `docs/interface-modules.md`.
- Le polling des listes partage seulement son cycle technique via `src/hooks/use-polling.ts` et `src/lib/polling.ts`; URL, erreurs, validation des réponses et cadences restent dans les hooks Tasks/Sequences. Les mutations suspendent et invalident le polling avant réconciliation, puis ne rechargent immédiatement que les données manquantes. Les actions de queue partagent `runs/use-queue-actions.ts`, avec confirmations propres aux écrans. Voir `docs/interface-data-flow.md` pour la matrice des mutations et les mesures reproductibles.
- `src/modules/projects/settings/` possède les sections dépôt/initialisation, exécution/Python, variables locales, publication et suppression. Les brouillons persistent entre onglets internes, mais un changement de dépôt remonte les Settings et annule les lectures précédentes. Une variable déjà configurée laissée vide est omise du payload pour conserver sa valeur.
...
- SQLite/Drizzle, la gestion des Projects (renommage, archivage local et suppression protégée des Runs actifs), l'initialisation `.tasker/`, les Instructions, la mémoire opérationnelle versionnée `.tasker/LESSONS.md`, les Settings Git/exécution et la tab Agents sont implémentés. Codex lit directement les Lessons depuis son worktree; le runner ne copie jamais leur contenu dans le prompt.
- La CLI `agenttasker init` initialise un dépôt Git de façon idempotente, installe le skill canonique, ajoute uniquement `/.tasker/logs/` au `.gitignore` et dépose une demande d’enregistrement hors dépôt. L’application importe ces demandes dans SQLite lors du chargement des Projects, sans coupler la CLI au schéma de base. `agenttasker sequence sync --id <sequence-id>` ou `--all` migre explicitement les historiques locaux vérifiables vers les checkpoints distants; `--dry-run` n’écrit rien et `--database` cible une base SQLite non standard. Voir `docs/cli-onboarding.md`.
...
- La tab Sequences fournit le CRUD versionné `.tasker/sequences/`, un éditeur de `SequenceSteps` ordonnées, une stratégie de PR configurable (une PR finale, des PR empilées ou des PR indépendantes par étape), Run Sequence, la progression par étape et l’historique séparé. Le mode indépendant repart de la branche de base après chaque publication et peut poursuivre après un échec avec un seuil d’échecs consécutifs configurable (2 par défaut); il exige la publication GitHub. Les autres modes arrêtent par défaut au premier échec. Une **Pause** arrête Codex de manière vérifiée, conserve le worktree local et permet **Reprendre** sur cette même machine : les étapes certifiées ne sont jamais rejouées et un fichier partiel reste disponible pour l’étape interrompue; la pause n’est pas portable. Lorsque le push Git est activé, chaque Step certifié publie aussi la branche de Run et un checkpoint réduit versionné sur la branche distante `agenttasker/state`; un autre environnement peut choisir **Reprendre le checkpoint**, qui recrée un worktree depuis le commit vérifié et n’exécute que le suffixe. Dans le mode indépendant, l’écriture finale de ce checkpoint réutilise le dernier commit distant certifié plutôt que la branche locale remise à la base; une synchronisation portable en erreur après des étapes certifiées est un avertissement, jamais un faux `FAILED`. Les logs, PID, sorties brutes et secrets restent en SQLite locale. Depuis le Sheet d’une étape réussie avec commit mais sans PR, **Publier une PR brouillon** crée de façon idempotente une PR GitHub en brouillon, ou retrouve une PR ouverte existante, sur une branche pointant exactement ce commit, sans relancer Codex ni les validations; le Run doit être terminal et son arrêt vérifié. Après un succès Codex suivi d’une validation échouée, **Reprendre** crée un Run lié qui réutilise le worktree préservé, rejoue les validations et poursuit sans rappeler Codex pour l’étape terminée. Après une fin Codex récupérable avant validation, il relance plutôt Codex uniquement pour l’étape échouée dans ce même worktree, en lui fournissant aussi un extrait borné du dernier compte rendu local comme contexte non fiable. Les garde-fous exigent un arrêt vérifié, les coordonnées Git exactes et une frontière d’étapes vérifiable. Voir `docs/sequence-architecture.md`.
- Lorsqu’une nouvelle étape est ajoutée après un Run de Sequence, **Exécuter** crée une continuation depuis le dernier préfixe d’étapes certifiées compatible : les étapes précédemment réussies et leurs commits restent immuables, y compris si le Run a ensuite échoué ou a été annulé. Un worktree neuf est créé depuis le dernier commit vérifié du Run source et Codex ne reçoit que la première étape non certifiée et les suivantes. L’interface annonce clairement ce choix; voir `docs/sequence-architecture.md`.
- Le Sheet live est un Run Inspector : `src/modules/runs/run-inspector/` normalise les événements JSONL Codex, regroupe le cycle des items et rend une timeline humaine avec détails techniques secondaires. Pour un Run ou une SequenceStep, il présente le pipeline dans l’ordre réel — environnement, Codex, validations, Git, publication, résultat — et limite « Activité » aux événements Codex; les commandes gérées et les données brutes restent dans leurs sections dédiées. Les deltas App Server à haute fréquence et les duplications stdout JSON-RPC sont supprimés à la source; les résultats image base64 sont remplacés par leurs métadonnées avant persistance; les anciens Runs sont filtrés avant l’état React et les listes sont rendues progressivement. Le protocole supporté et ses limites sont documentés dans `docs/codex-run-events.md`.
...
- Le succès exige une préparation réussie lorsqu’activée, une sortie structurée positive, toutes les validations configurées et les contrôles Git. Si Codex termine avec un dernier message non conforme au schéma, le runner demande une seule correction JSON dans le même thread, sans commande ni modification; un second résultat invalide échoue et préserve le worktree. Le worker commit puis applique la publication `[git]` optionnelle du Project : push vérifié et PR GitHub idempotente, brouillon par défaut lorsqu’elle est activée. Chaque Sequence choisit dans `sequence.toml` entre une PR après toute la Sequence et des PR distinctes empilées après chaque étape qui produit un commit. Les URLs de PR et les pushs sont persistés sur le Run et, en mode par étape, sur les `SequenceStepRun`; aucun auto-merge n’est effectué. Les échecs/annulations préservent le worktree. Au redémarrage, le worker vérifie l’arbre de processus enregistré avant de reprendre la queue ; lorsqu’une ancienne terminaison ne peut plus être vérifiée automatiquement, le Sheet exige une confirmation locale explicite avant la reprise. Voir `docs/task-runner-architecture.md` pour les politiques de reprise et de nettoyage.
...
- Ranger les composants, hooks, types et utilitaires d’une fonctionnalité dans son module `src/modules/<domaine>/`; créer des sous-dossiers locaux seulement si leur volume le justifie. Réserver `src/components/ui/` et `src/components/reui/` aux primitives, `src/hooks/` et `src/lib/` aux fonctions réellement utilisées par plusieurs modules, et `src/types/` aux contrats partagés UI/API/serveur.
- Garder `src/app/` pour les routes et la composition et `backend/` pour les services serveur. `projects` compose les vues des autres domaines; `app-shell` utilise le contexte Projects pour la navigation; `tasks` et `sequences` utilisent `runs` sans dépendre l’un de l’autre. Ne jamais importer de service serveur exécutable dans un module client.
...
- La récupération locale est un dernier garde-fou, pas la réponse normale à un faux échec reproductible. Lorsqu’une exception opérationnelle est comprise, corriger sa cause, ajouter une régression et documenter le comportement avant de recommander une récupération; celle-ci reste réservée aux états réellement inconnus ou non certifiables.
