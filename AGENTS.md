...
- Les Settings d’exécution peuvent déclarer `python_min_version` (par exemple `3.11`) sans chemin machine. À chaque Run Task ou Sequence, le worker diagnostique `py -<version>` / `py -3` sous Windows puis `python` / `python3`, conserve chemins et versions dans l’historique local, et préfixe le dossier de l’interpréteur résolu au `PATH` de Codex et des commandes gérées. L’absence de réglage laisse Python optionnel.
