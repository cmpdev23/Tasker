...
- Le Sheet live est un Run Inspector : `src/components/run-inspector/` normalise les événements JSONL Codex, regroupe le cycle des items et rend une timeline humaine avec détails techniques secondaires. Les deltas App Server à haute fréquence et les duplications stdout JSON-RPC sont supprimés à la source; les résultats image base64 sont remplacés par leurs métadonnées avant persistance; les anciens Runs sont filtrés avant l’état React et les listes sont rendues progressivement. Le protocole supporté et ses limites sont documentés dans `docs/codex-run-events.md`.
