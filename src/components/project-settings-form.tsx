...
export function ProjectSettingsForm({
  ...
  const fetchExecutionSettings = useCallback((signal?: AbortSignal) => {
    return fetch(`/api/projects/${project.id}/execution`, { signal }).then(async (response) => {
      ...
      setEnvironmentVariables((localRuntime?.environmentVariables ?? []).map((entry) => ({
    ...
    try {
      const response = await fetch(`/api/projects/${project.id}/execution`, {
        ...
        body: JSON.stringify({
          ...
          localRuntime: {
            pythonExecutable: pythonRuntimeMode === "explicit" ? localPythonExecutable.trim() || null : null,
            environmentVariables: environmentVariables.map((entry) => ({
              name: entry.name,
              ...(!entry.configured || entry.value !== "" ? { value: entry.value } : {}),
            })),
          },
        }),
      });
      ...
      setEnvironmentVariables((data.localRuntime?.environmentVariables ?? []).map((entry: { name: string; configured: true }) => ({
        id: crypto.randomUUID(),
        name: entry.name,
        value: "",
        configured: entry.configured,
      ...
      if ((pythonRuntimeMode === "explicit" || Boolean(pythonMinVersion.trim())) &&
        ...
        toast.success("Execution settings, local runtime, and environment variables saved.");
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-6">
          ...
          <dl className="flex flex-col">
            <div className="grid items-center gap-1 px-4 py-3 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:gap-5">
              ...
              <dd className="flex items-center gap-2 text-sm text-foreground">
                {inspection?.isTaskerInitialized ? (
                  <Badge tone="success" variant="dot-outline">Initialized</Badge>
                ) : (
              ...
              <>
                ...
                <div className="grid items-center gap-1 px-4 py-3 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:gap-5">
                  ...
                  <dd className="flex items-center gap-2 font-mono text-xs text-foreground">
                    <span className="rounded bg-muted px-2 py-0.5 font-medium">.tasker/</span>
                    <span className="text-muted-foreground">(project.toml, instructions.md, agents/, tasks/)</span>
                  </dd>
                </div>
              </>
            )}
      ...
      <section aria-labelledby="execution-settings-title" className="flex w-full flex-col gap-3">
        <div className="flex flex-col gap-3 px-1 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex min-w-0 flex-col gap-1">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Project configuration</p>
            <h2 id="execution-settings-title" className="text-base font-semibold text-foreground">Execution settings</h2>
            <p className="text-sm text-muted-foreground">Runtime, secrets, validation scripts, and advanced Run behavior.</p>
          </div>
          <Button
            type="button"
        ...
        <div className="overflow-hidden rounded-xl border bg-card shadow-xs">
          {!inspection?.isTaskerInitialized ? (
            ...
            <Tabs defaultValue="runtime" className="gap-0">
              <div className="border-b bg-muted/25 px-3 pt-2">
                <TabsList variant="line" className="h-auto max-w-full flex-wrap justify-start gap-1" aria-label="Execution settings sections">
                  <TabsTrigger value="runtime" className="flex-none px-3 py-2 text-xs">Packages &amp; Python</TabsTrigger>
                  <TabsTrigger value="environment" className="flex-none px-3 py-2 text-xs">Environment variables</TabsTrigger>
                  <TabsTrigger value="scripts" className="flex-none px-3 py-2 text-xs">Scripts</TabsTrigger>
                  <TabsTrigger value="advanced" className="flex-none px-3 py-2 text-xs">Advanced</TabsTrigger>
                </TabsList>
              </div>
              ...
              <TabsContent value="runtime" className="m-0">
                <dl className="flex flex-col">
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-center sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Package manager</dt>
                  ...
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-start sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Python runtime</dt>
                    <dd className="flex max-w-lg flex-col gap-3">
                      <Input
                        ...
                        inputMode="decimal"
                      />
                      <p className="text-xs text-muted-foreground">Portable minimum stored in .tasker/project.toml. Leave empty when Python is optional.</p>
                      <div className="flex flex-col gap-2 rounded-md border p-3">
                        ...
                        <p className="text-xs text-muted-foreground">The selected path is machine-local SQLite state. It is never written to the repository.</p>
                      </div>
                    </dd>
                  </div>
                  ...
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-start sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Runtime preflight</dt>
                        ...
                        return (
                          <div key={key} className="flex min-w-0 flex-col gap-1">
                            <div className="flex min-w-0 items-center gap-2">
                              <Badge tone={status == null ? "outline" : status.available ? "success" : "destructive"} variant="dot-outline">
              ...
              <TabsContent value="environment" className="m-0">
                <div className="flex max-w-3xl flex-col gap-4 p-4 sm:p-5">
                  <div className="flex items-start gap-2 rounded-md border border-amber-500/25 bg-amber-500/5 p-3 text-xs text-muted-foreground">
                    ...
                    <p>Local values injected into Codex, dependency installation, and validations. Values are encrypted at rest, never written to the worktree, and never returned after saving.</p>
                  </div>
                  <div className="flex flex-col gap-2">
                    {environmentVariables.length === 0 && (
                      <div className="rounded-md border border-dashed px-4 py-6 text-center text-sm text-muted-foreground">No local environment variables configured.</div>
                    )}
                    {environmentVariables.map((entry) => (
                      <div key={entry.id} className="grid gap-2 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)_auto]">
                        <Input
                          value={entry.name}
                          onChange={(event) => setEnvironmentVariables((current) => current.map((candidate) =>
                            candidate.id === entry.id
                              ? { ...candidate, name: event.target.value, configured: candidate.name.toUpperCase() === event.target.value.toUpperCase() && candidate.configured }
                              : candidate
                          ))}
                        ...
                        <Input
                          ...
                          placeholder={entry.configured ? "Saved — leave blank to keep" : "Value"}
                        ...
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          onClick={() => setEnvironmentVariables((current) => current.filter((candidate) => candidate.id !== entry.id))}
                  ...
                  <Button
                    ...
                    onClick={() => setEnvironmentVariables((current) => [...current, {
                      id: crypto.randomUUID(),
                      name: "",
                      value: "",
                      configured: false,
                  ...
                  <p className="text-xs text-muted-foreground">Removing a row deletes that local value when you save. Variable names are recorded in Run diagnostics; direct values are redacted.</p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="scripts" className="m-0">
                <dl className="flex flex-col">
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-center sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Install dependencies</dt>
                    <dd className="flex flex-col gap-1.5">
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">Runs lifecycle scripts and may use the network. Enable only for repositories you trust.</p>
                    </dd>
                  </div>
                  ...
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-start sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Validation scripts</dt>
                    <dd className="flex flex-col gap-1.5">
                      <Textarea
                      ...
                      />
                      <p className="text-xs text-muted-foreground">One package.json script per line. All scripts must pass before AgentTasker commits.</p>
                    </dd>
                  </div>
                  ...
                  <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-start sm:gap-5">
                    <dt className="text-sm font-medium text-muted-foreground">Resolved commands</dt>
              ...
              <TabsContent value="advanced" className="m-0">
                <div className="flex flex-col">
                  <dl className="flex flex-col">
                    <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-start sm:gap-5">
                      <dt className="text-sm font-medium text-muted-foreground">Timeouts</dt>
                  ...
                  <div className="flex flex-col gap-3 bg-muted/15 px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">Push verified Run branches and optionally open GitHub pull requests.</p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    <dl className="flex flex-col">
                      ...
                      <div className="grid gap-2 px-4 py-4 sm:grid-cols-[minmax(10rem,0.95fr)_minmax(0,1.35fr)] sm:items-center sm:gap-5">
                        ...
                        <dd className="flex min-w-0 items-center gap-2 text-xs">
                          <Badge tone={gitRuntime?.authenticated ? "success" : gitRuntime?.available ? "warning" : "destructive"} variant="dot-outline">
                  ...
                  <div className="border-t-4 border-t-destructive/25 bg-destructive/[0.025] p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-5">
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="text-xs text-muted-foreground">Supprime le projet et son historique de la base locale AgentTasker. Le repository sur disque ne sera pas supprimé.</p>
                      </div>
                      <Button
                        type="button"
                        variant="destructive"
