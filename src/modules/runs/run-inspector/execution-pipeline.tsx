...
export function ExecutionPipeline({
  ...
  return (
    <section aria-labelledby="execution-pipeline-title" className="space-y-3">
      <div>
        <h2 id="execution-pipeline-title" className="text-sm font-medium">
          Progression de l’étape
        </h2>
        <p className="mt-1 text-xs text-muted-foreground">
          AgentTasker vérifie chaque phase avant de poursuivre la suivante.
        </p>
      </div>
...
export function EnvironmentPreparation({
  ...
  const stage = pipelineStages(run, commands, sequenceStep).find(
    (item) => item.id === "environment",
  )!;
  return (
    <section
      aria-labelledby="environment-preparation-title"
      className="space-y-3"
    >
      <div className="flex gap-3">
        ...
        <div>
          <h2
            id="environment-preparation-title"
      ...
      {commands.length > 0 && (
        <ProjectCommands
          commands={commands}
          heading="Commandes de préparation"
          titleId="preparation-commands-title"
          commandLabel="Installation"
        />
      )}
    </section>
  );
...
export function CodexCompletion({
  run,
  commands,
  sequenceStep,
}: {
  ...
  const codexFinished = ["success", "failed", "skipped"].includes(
    stage.status,
  );
  const title = codexFinished ? "Fin de Codex" : "Travail de Codex";
  const titleId = codexFinished
    ? "codex-completion-title"
    : "codex-work-title";
  const detail =
    stage.status === "success"
      ? "Codex a terminé correctement : code de sortie 0 et résultat structuré SUCCESS."
      : stage.status === "running"
        ? "Codex travaille dans le worktree isolé."
        : stage.status === "failed"
          ? "Codex n’a pas produit une terminaison valide pour cette étape."
          : stage.status === "skipped"
            ? "Codex a été arrêté avant la fin de cette étape."
            : "Codex démarrera après la préparation de l’environnement.";
  return (
    <section
      aria-labelledby={titleId}
      className="border-l-2 border-muted py-1 pl-4"
    >
      <div className="flex gap-3">
        <StatusIcon status={stage.status} className="mt-0.5" />
        <div className="min-w-0">
          ...
          {(run.exitCode !== null || result?.status) && (
            <p className="mt-1 text-xs text-muted-foreground">
              Code de sortie : {run.exitCode ?? "non disponible"}
              {result?.status ? ` · Résultat structuré : ${result.status}` : ""}
            </p>
          )}
        </div>
      </div>
...
export function GitFinalization({
  run,
  commands,
  sequenceStep,
  publicationBranch,
  ...
  const publicationEnabled =
    config.push === true ||
    config.createPullRequest === true ||
    Boolean(run.pushedAt || run.pullRequestUrl);
  const showPublication = publicationEnabled || Boolean(onPublishDraft);
  const deferredSequencePublication =
    sequenceStep &&
    run.kind === "SEQUENCE" &&
    !["after_each_step", "independent_after_each_step"].includes(
      config.sequencePullRequestStrategy ?? "",
    ) &&
    publicationEnabled;
  ...
  return (
    <section aria-labelledby="git-finalization-title" className="space-y-4">
      <div className="flex gap-3">
        ...
        <div>
          <h2 id="git-finalization-title" className="text-sm font-medium">
            Vérification Git et intégration
          </h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            {git.status === "success"
              ? "Le diff Git a été vérifié avant la finalisation de l’étape."
              : git.status === "running"
                ? "AgentTasker vérifie le diff et finalise les changements."
                : git.status === "failed"
                  ? "La vérification Git ou la finalisation n’a pas abouti."
                  : "Cette vérification sera exécutée après les validations."}
          </p>
        </div>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        <p className="rounded-md border px-3 py-2 text-xs">
          ...
          <br />
          <span className="break-all font-mono">{branch ?? "En attente"}</span>
        </p>
        <p className="rounded-md border px-3 py-2 text-xs">
          ...
          <span className="break-all font-mono">
            {run.commitHash ??
              (git.status === "skipped" ? "Aucun commit requis" : "En attente")}
          </span>
        </p>
      </div>
      {showPublication && (
        <div className="space-y-2 border-t pt-4">
          <div className="flex gap-3">
            <StatusIcon
              status={
                deferredSequencePublication ? "pending" : publication.status
              }
              className="mt-0.5"
            />
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                {deferredSequencePublication
                  ? "La branche et la PR seront publiées après la dernière étape de la Sequence."
                  : publication.status === "success"
                    ? "La publication configurée a été réalisée."
                    : publication.status === "failed"
                      ? "La publication configurée n’a pas abouti."
                      : publicationEnabled
                        ? "La publication sera lancée après la finalisation Git."
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="rounded-md border px-3 py-2 text-xs">
              ...
              {run.pullRequestUrl ? (
                ...
                >
                  Ouvrir la PR <ExternalLinkIcon className="size-3" />
                </a>
              ) : deferredSequencePublication ? (
                "Prévue à la fin de la Sequence"
              ) : (
                "En attente"
              )}
            </div>
          ...
          {onPublishDraft && !run.pullRequestUrl && (
            <div className="rounded-md border border-dashed px-3 py-3 text-xs">
              ...
              <p className="mt-1 leading-5 text-muted-foreground">
                Crée une branche pointant sur ce commit exact et ouvre une pull
                request GitHub en brouillon, sans rejouer Codex ni les
                validations.
              </p>
              <Button
                type="button"
