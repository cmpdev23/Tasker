...
export function ProjectTasksView(props: {
  ...
  return (
    <TasksView
      key={`${props.project.id}:${props.project.repositoryPath}`}
      {...props}
    />
  );
}
...
function TasksView({
  ...
  const mergedRuns = new Map(
    [...olderRuns, ...runs].map((run) => [run.id, run]),
  );
  const orderedRuns = [...mergedRuns.values()].sort(
  ...
  async function loadOlderRuns() {
    ...
    setOlderError(null);
    try {
      const before = orderedRuns.reduce(
        (oldest, run) => (run.createdAt < oldest ? run.createdAt : oldest),
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      {queueStatus &&
        ["BLOCKED_RECOVERY", "BLOCKED_PROCESS", "RECOVERY_REQUIRED"].includes(
          queueStatus.state,
        ) && (
          <QueueStatusPanel
            queue={queueStatus}
            ...
            onDelete={
              queueStatus.canRecover && queueStatus.blocker
                ? () => void removeBlockingRun()
                : undefined
            }
          />
        )}
      <Frame stacked spacing="sm">
        <FrameHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <FrameDescription>
              Planifiez le travail de Codex et suivez chaque exécution.
            </FrameDescription>
          </div>
          <Button
            variant="light"
            disabled={!project.repositoryPath || loading || !!taskError}
        ...
        <FramePanel className="p-0">
          {!project.repositoryPath ? (
            <div className="flex flex-col items-center gap-3 p-10 text-center">
              ...
              <p className="text-sm">
                Configurez le repository du projet pour créer des tâches.
              </p>
              {onNavigateToSettings && (
            ...
            <>
              ...
              {taskError && (
                <div
                  role="alert"
                  className="mx-4 mb-3 flex flex-wrap items-center gap-3 rounded-lg border border-destructive/20 bg-destructive/5 p-4 text-sm"
                >
                  <p className="flex-1 text-destructive">
                    {taskError}
                    {tasks.length > 0 &&
                      " La liste affichée peut être obsolète."}
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
              ...
              {loading ? (
                ...
                !taskError && (
                  <div className="flex flex-col items-center gap-3 p-12 text-center">
                    <ListTodoIcon className="size-8 text-muted-foreground" />
                    <h3 className="text-sm font-medium">
                      Votre première tâche
                    </h3>
                    <p className="max-w-sm text-sm text-muted-foreground">
                      Décrivez un travail à réaliser, puis lancez-le
                      manuellement ou selon une planification.
                    </p>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <caption className="sr-only">
                      Tâches du projet, planification et dernière exécution
                    </caption>
                      ...
                      {tasks.map((task) => {
                        ...
                        return (
                          ...
                          >
                            <th
                              scope="row"
                              className="min-w-44 px-4 py-2.5 font-normal"
                            >
                            ...
                            <td className="min-w-40 px-4 py-2.5 text-xs leading-normal">
                              {scheduleLabel(task)}
                              {task.schedule.startsAt &&
                            ...
                            <td className="min-w-36 px-4 py-2.5">
                              {last ? (
                                <button
                                  ...
                                  onClick={() => setSelectedRun(last)}
                                  aria-label={`Ouvrir le dernier Run de ${task.name}`}
                                >
                                  <RunStatusBadge status={last.status} />
                                  <span className="mt-1 block text-xs text-muted-foreground">
                                    {formatRunDate(
                                      last.startedAt || last.queuedAt,
                                    )}
                                  </span>
                                </button>
                              ) : (
                            ...
                            <td className="min-w-72 px-4 py-2.5">
                              <ButtonGroup className="ml-auto !w-72 shrink-0">
                                ...
                                >
                                  <span
                                    aria-hidden="true"
                                    className={`size-1.5 shrink-0 rounded-full ${state.dotClass}`}
                                  />
                                ...
                                <Button
                                  ...
                                  aria-label={`${isRerunnableRun(last?.status ?? "") ? "Réexécuter" : "Exécuter"} ${task.name}`}
                                  title={
                                    isRerunnableRun(last?.status ?? "")
                                      ? "Réexécuter"
                                      : "Exécuter"
                                  }
                                >
      ...
      {project.repositoryPath && (
        <Frame stacked spacing="sm" id="task-run-history">
          <FrameHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <FrameTitle>Historique des Runs</FrameTitle>
              <FrameDescription>
                Toutes les exécutions du projet · actualisation toutes les 3 s
              </FrameDescription>
            </div>
          </FrameHeader>
          <FramePanel className="p-0">
            {runError && (
              ...
              >
                {runError} Nouvelle tentative automatique…
              </p>
            )}
            {loading ? (
      ...
      {selectedRun && (
        <TaskRunSheet
          ...
          rerunning={starting === selectedRun.taskId}
          onRerun={
            tasks.some((task) => task.id === selectedRun.taskId)
              ? () => void runNow(selectedRun.taskId)
              : undefined
          }
          onClose={() => {
            setSelectedRun(null);
      ...
      >
        <DialogContent showCloseButton={!deleting}>
          <DialogHeader>
            ...
            <DialogDescription>
              La définition de « {deleteTask?.name} » sera supprimée du
              repository. L’historique des Runs reste consultable.
            </DialogDescription>
          </DialogHeader>
          {deleteError && (
